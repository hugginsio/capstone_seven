...
export class MenuLandingComponent implements OnInit {
  ...
  constructor(private readonly electronService: ElectronService, private readonly soundService: SoundService) {}
  ...
  quitElectronHost(): void {
    if (this.appConfig.production) {
      this.electronService.isElectron
        ...
        : console.warn("Electron not detected, close() not executed.");
    } else {
      this.electronService.isElectron
        ? alert("Electron running in DEV, close() will not be executed.")
        : console.warn("Electron not detected, close() not executed.");
    }
  }
}
