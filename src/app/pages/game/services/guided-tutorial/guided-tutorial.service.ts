...
export class GuidedTutorialService {
    ...
    if (this.humanPlayer === "1") {
      switch (m) {
        ...
        case 14:
          ...
          break;
        case 15:
          piece = "4"; // yellow trade
          break;
        case 16:
          piece = "1"; // yellow trade
          break;
        case 17:
          piece = "4"; // red trade
          break;
    ...
    if (this.humanPlayer === "1") {
      ...
      /*else if(m === 25 && s === 23 && piece === 'endTurnBtn')
      {
        ...
        this.snackbarService.add({
          message: "Now don't be clickin' just anywhere! Please follow these here instructions.",
        });
        validMove = false;
      }
    }
  ...
  messageOne(): string {
    const message =
      'Welcome to the mines! <br><br> Let’s walk you through a few steps to get y’all on the right foot with this here underground duel.<br><br>Click the "Next" button to start the tutorial.';
  ...
  messageTwo(): string {
    // message 2
    const message =
      "Learn some minin' lingo before we get going:<br><br>These squares here which make up the mine are called Sites.<br><br>These Sites tell ya which Gems they give and how many Tools they can have on their corners.";
  ...
  messageThree(): string {
    const message =
      "On each corner of a Site is a place for a Pickaxe or Drill which are what we call \"Tools.\"<br><br>Each Tool will give ya one Gem per turn from each of the touchin' Sites.<br><br>These here Paths on the sides of each Site are how ya get around to expandin' your operation.";
  ...
  messageFour(): string {
    ...
    if (this.humanPlayer === "1") {
      // message 3
      message =
        "Start the competition by puttin' down a Tool and a connectin' Path anywhere in the mine.<br><br>Click the indicated Tool and Path spaces to make your move.";
      if (this.moveNum === 1) {
  ...
  messageFive(): string {
    ...
    if (this.humanPlayer === "1") {
      // message 5
      message =
        'Settled on your move? Click "End Turn" to keep the game movin\'.<br><br>Then click "Next" to continue.';
  ...
  messageSix(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        'Now the other Player selects their moves...<br><br>You see these here Gems on each Site? The number of Gems is the maximum amount of Tools the Site can support.<br><br>If you and your opponent’s number of Tools on a Site are more than the maximum, then that Site is "Tuckered Out" and no one\'s gettin’ Gems from it.';

      if (
        this.gameManager.getBoard().nodes[15].getOwner() === "NONE" &&
        this.gameManager.getBoard().branches[28].getOwner() === "NONE"
      ) {
        this.AImove(";15;28");
        this.highlightNext();
      }
  ...
  messageSeven(): string {
    ...
    if (this.humanPlayer === "1") {
      ...
      if (
        this.gameManager.getBoard().nodes[3].getOwner() === "NONE" &&
        this.gameManager.getBoard().branches[7].getOwner() === "NONE"
      ) {
        this.AImove(";3;7");
        this.highlightNext();
      }
  ...
  messageNine(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        'Go ahead and click the "Undo" button twice to take back your last two moves.<br><br>Finish your turn by placin\' the highlighted pieces and clicking "End Turn."<br><br>Then click "Next" to continue.';
      if (this.moveNum === 6) {
        this.highlightManager();
        this.freezeNext = true;
      }
  ...
  messageTen(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        "These Gems ain’t just for lookin’ at. You can expand your operation by buyin' more Tools and Paths with your Gems.<br><br>Take a gander at the Price Card to remind yourself of their costs.";
  ...
  messageEleven(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        "Lookin’ to trade some Gems?<br>When it’s your turn, you can make a Trade of 3 Gems for 1 Gem of any other type.<br><br>You can only Trade once per turn.<br><br>Gems ya don't spend are saved in your stash at the bottom of the screen.";
  ...
  messageTwelve(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        "The other Player is up again. They're gonna get Gems at the start of each turn from all their Tools. They can make any amount of moves as long as they got the Gems to pay up.<br><br> Your opponent has traded 3 yellow Gems for a blue so they could afford two Paths on this turn.";

      if (
        this.gameManager.getBoard().branches[18].getOwner() === "NONE" &&
        this.gameManager.getBoard().branches[17].getOwner() === "NONE"
      ) {
  ...
  messageThirteen(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        "Now it’s your turn again!<br><br>Take a gander at your stash to see what you can do next.<br><br>Any new Paths or Tools have gotta be connected to one of your other Paths.";
      this.highlightNext();
    }
    return message;
  }
  ...
  messageFifteen(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        "Now you have the longest Path in the duel. This means you got 2 more points on the way to your goal of 10 to win. <br><br>But beware! This can be snatched right out from under you by your opponent.";
      this.highlightNext();
    }
    return message;
  }
  ...
  messageSixteen(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        "Now it’s the opponent's turn to take a crack at it.<br>They connect their two separate Paths into one big one and take those 2 points from you!";
      ...
      if (
        this.gameManager.getBoard().branches[12].getOwner() === "NONE" &&
        this.gameManager.getBoard().branches[31].getOwner() === "NONE"
      ) {
  ...
  messageSeventeen(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        "At the bottom of the screen is where you can see the current standings for the duel.<br><br>Both y’all got 1 point for each Tool ya placed, and the opponent’s got 2 more for havin' the longest Path.";
      const player1 = document.getElementById("player1");
      const player2 = document.getElementById("player2");
  ...
  messageEighteen(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        "Now you got enough Gems to place a Tool!<br><br>Remember, you cain’t just be mining in a place where you don't have the Paths to get you there!<br><br>Place a Tool in the highlighted space and earn yourself another point.";
      if (this.moveNum === 13) {
        this.highlightManager();
        this.freezeNext = true;
      }
  ...
  messageNineteen(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        'Try your hand at Tradin\' Gems so ya can place two more Paths.<br><br>Click the "Trade" button. Then select 1 yellow, 1 red, and 1 more yellow to trade away. Then select 1 blue to trade for and confirm your trade.<br><br>Go on and place those two highlighted Paths before clickin\' "End Turn" and then "Next".';
      if (this.moveNum === 14) {
        this.highlightManager();
        this.freezeNext = true;
      }
  ...
  messageTwenty(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        "Alright, one last turn for your opponent before we turn y’all loose in the mine to finish up this here friendly competition.";
  ...
  messageTwentyOne(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        "See how these Sites changed when the opponent surrounded them with Paths on all sides?<br><br>This is how you can get exclusive mining rights to these Sites!<br><br>These captured Sites are gonna give your opponent 1 point a piece. Plus, only your opponent can get Gems from those Sites now.";
  ...
  messageTwentyFour(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        'Need more help or want to change the settings? Click the "Menu" button in the upper-left corner for more options.';
      this.highlightNext();
    }
    return message;
  }
  ...
  messageTwentyFive(): string {
    ...
    if (this.humanPlayer === "1") {
      message =
        'Seems like you’ve got a good handle on how we do things down here in the mine! Time to see how you do on your own.<br><br>First player to 10 points wins gold. Good luck, Prospector!<br><br>Click "Next" to end the Tutorial then go on an\' click "End Turn" when you\'re ready.';
      this.highlightNext();
    }
    return message;
  }
